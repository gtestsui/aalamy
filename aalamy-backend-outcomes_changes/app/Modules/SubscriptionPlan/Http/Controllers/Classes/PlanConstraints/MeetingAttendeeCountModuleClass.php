...
class MeetingAttendeeCountModuleClass extends BasePlanConstraintsClass
{
    ...
    protected $requestedAttendeeCount;
...
//    }
    ...
    public static function createByOwner(User $user)
    {
        ...
        $static->module = Module::where(
                'identify',getModuleIdentify('meeting_attendees_count')
            )
     ...
     */
    public function canAddThisMeetingAttendeeCount(){

        if(!isset($this->requestedAttendeeCount))
            throw new ErrorMsgException('requestedAttendeeCount should initialized before using it');
...
//        $currentCount = $this->getCurrentMeetingsCount();
        if($this->requestedAttendeeCount >= $availableQuantityFromUserPlan)
            return false;

        return true;
    }

    public function setRequestedAttendeeCount($requestedCount){
        $this->requestedAttendeeCount = $requestedCount;
        return $this;
    }
    ...
    public function check(){
        ...
        if($this->canAddThisMeetingAttendeeCount())
            return true;
