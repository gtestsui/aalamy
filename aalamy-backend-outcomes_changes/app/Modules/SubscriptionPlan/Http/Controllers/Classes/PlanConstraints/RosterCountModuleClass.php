...
class RosterCountModuleClass extends BasePlanConstraintsClass
{
    ...
    public static function createByOwner(User $user)
    {
        ...
        $static->module = Module::where(
                'identify',getModuleIdentify('rosters_count')
            )
     ...
     */
    public function canAddMoreRosters(){
        $this->checkAndSetUserSubscribe();
        $availableQuantityFromUserPlan = $this->getAvailabeQuantity();
        $myRostersCount = $this->getCurrentRostersCount();
        if($myRostersCount >= $availableQuantityFromUserPlan)
            return false;
    ...
    public function checkWithCustomizedErrorForTeacher($msg=null){
        ...
        $msg = $msg ?? transMsg('max_count_of_school_rosters',ApplicationModules::SUBSCRIPTION_PLAN_MODULE_NAME);
