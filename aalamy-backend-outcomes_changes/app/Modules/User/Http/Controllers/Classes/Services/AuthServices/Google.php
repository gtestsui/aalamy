...
class Google implements AuthByServiceInterface
{
...
//        $this->access_token = $access_token;
        ...
        try {
            $this->info = Socialite::driver('google')
                ->userFromToken($authData->service_access_token);

        }catch (\Exception $e){
            $this->info = null;
            throw new ErrorMsgException('some thing went wrong with google account');
        }
...
//            ->where('email',$this->getEmail())
            ->where('account_id',config('User.panel.login_services.google'))
            ->first();
        if(is_null($user))
